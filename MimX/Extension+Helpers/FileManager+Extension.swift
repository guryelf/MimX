...
extension FileManager{
    ...
    func createFolderAndSave(name:String,dataToWrite:[Data],fileExtension:String) -> [URL]{
        let folderName = name
        var url = [URL]()
        let fileManager = FileManager.default
        if let documentsDirectory = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first {
            ...
            do {
                ...
                dataToWrite.forEach {
                    let fileURL = folderURL.appendingPathComponent("name\(UUID().uuidString).\(fileExtension)")
